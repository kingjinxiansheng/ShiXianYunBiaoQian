    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        for (int i = 0; i < count; i++) {
            ...
            if (lineWidth + childWidth > sizeWidth - getPaddingLeft() - getPaddingRight()) {
                ...
                child.setTag(new Location(left, top + height, childWidth + left - childHorizontalSpace-20, height + child.getMeasuredHeight() + top));
            } else {// 否则累加值lineWidth,lineHeight取最大高度
                child.setTag(new Location(lineWidth + left, top + height, lineWidth + childWidth - childHorizontalSpace + left-20, height + child.getMeasuredHeight() + top));
